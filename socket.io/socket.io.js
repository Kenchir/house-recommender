...
io.on('connection',(socket)=>{
      ...
      //Add response to invalid on client side
       ...
       //Send message if online
                 if (clients[isValid.value.to]){
                         ...
                         io.sockets.connected[clients[isValid.value.to].socket].emit("receive-message", newMsg);
                         } else {
        ...
        Message.create(newMsg,function(err,saved){
            if(err){console.log(err)}
                ...
                Conversation.findOneAndUpdate({_id:id},{lastActivity:moment().valueOf()},function(err){
                    if(err){console.log(err)}
                    else{
    ...
    socket.on('read',function(readInfo,cb){
       ...
       Message.find({$and: [{
        to:readInfo.by},{from:readInfo.for },{ status:'unread' 
             ...
             console.log(data);
                     ...
                     //   var house_loc;
                        ...
                        let newpromise= new Promise((resolve,reject)=>{
                             requestify.get('https://maps.googleapis.com/maps/api/geocode/json?latlng='+data.lat+','+data.lng+'&location_type=APPROXIMATE'+'&key=AIzaSyBjsdFT4HpouHSdJX7fFPJg6Ym7re9ksuM')
